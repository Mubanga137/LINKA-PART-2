...
export function ShopCustomerHeader() {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-gradient-to-r from-emerald-600/95 via-green-600/95 to-teal-600/95 border-b border-white/20 shadow-lg">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            {/* Navigation and Logo */}
            <div className="flex items-center space-x-6">
              {/* Back to Dashboard */}
              <Link href="/customer-dashboard" className="text-white/80 hover:text-white transition-all duration-300 group">
            ...
            <nav className="hidden md:flex items-center space-x-4">
              <Link
                href="/shop"
                className="text-white hover:text-yellow-400 font-semibold transition-all duration-300 relative group py-2 px-4 rounded-lg bg-white/10 border border-white/20"
              >
                <Search className="h-4 w-4 mr-2 inline" />
                Browse Products
                <span className="absolute inset-x-0 -bottom-px h-px bg-gradient-to-r from-yellow-400 to-orange-500 scale-x-100"></span>
              </Link>
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <div className="absolute bottom-0 left-1/4 w-24 h-24 bg-gradient-to-br from-teal-400/20 to-emerald-500/20 rounded-full blur-xl animate-pulse-slow"></div>
        </div>
      </header>
      ...
      {isMobileMenuOpen && (
        <>
          ...
          {/* Mobile Menu Panel */}
          <div className="fixed top-0 right-0 z-50 h-full w-80 max-w-sm bg-gradient-to-br from-emerald-600 to-teal-700 shadow-2xl transform transition-transform duration-300 ease-in-out md:hidden">
            <div className="flex flex-col h-full">
              ...
              <div className="flex items-center justify-between p-6 border-b border-white/20">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-white/80">Shopping Mode</span>
                  </div>
                </div>
                <Button
                  variant="ghost"
              ...
              {/* Mobile Navigation Links - Shopping Focused */}
              <nav className="flex-1 overflow-y-auto py-6">
                <div className="space-y-2 px-6">
                  ...
                  <Link
                    href="/shop"
                    className="flex items-center gap-3 py-3 text-white hover:text-yellow-400 bg-white/10 rounded-lg px-3 transition-all duration-300 border border-white/20"
                    onClick={closeMobileMenu}
                  >
                  ...
                  >
                    ...
                    {totalItems > 0 && (
                      <Badge className="ml-auto h-5 w-5 p-0 flex items-center justify-center bg-yellow-500 text-black text-xs font-bold">
                        {totalItems}
                      </Badge>
                    )}
            ...
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
              <div className="absolute top-1/4 right-0 w-40 h-40 bg-gradient-to-br from-yellow-400/10 to-orange-500/10 rounded-full blur-2xl animate-float"></div>
              <div className="absolute bottom-1/3 left-0 w-32 h-32 bg-gradient-to-br from-teal-400/10 to-emerald-500/10 rounded-full blur-2xl animate-pulse-slow"></div>
            </div>
          </div>
        </>
      )}
